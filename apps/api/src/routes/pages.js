...
const MODULE_PAGES = [
  ...
  'admin-workers', 'admin-supervisors', 'admin-tickets',
  'admin-audit-logs', 'admin-qr-print', 'admin-attendance', 'admin-ai',
  'admin-electrical', 'admin-templates',
  'admin-assets', 'admin-failures', 'admin-alerts',
  'admin-predictions',
...
const MODULE_META = {
  ele:        { name: 'Kodspot Electrical',   short: 'Electrical',  color: '#d97706', desc: 'Electrical inspection & maintenance' },
...
async function pageRoutes(fastify) {
  ...
  fastify.get('/:orgSlug/:mod', async (request, reply) => {
    ...
    const org = await validateOrg(orgSlug);
    if (!org) return reply.code(404).send({ error: 'Organization not found' });
    if (!(org.enabledModules || ['ele']).includes(mod)) {
  ...
  fastify.get('/:orgSlug/:mod/manifest.json', async (request, reply) => {
    ...
    const meta = MODULE_META[mod] || MODULE_META.ele;
...
self.addEventListener('fetch', (event) => {
  ...
  // ── Backward compatibility: /{org}/{page} → redirect to /{org}/ele/{page} ──
  for (const page of MODULE_PAGES) {
    fastify.get('/:orgSlug/' + page, async (request, reply) => {
      const { orgSlug } = request.params;
      const org = await validateOrg(orgSlug);
      if (!org) return reply.code(404).send({ error: 'Organization not found' });
      // Default module is ele for backward compatibility
      return reply.redirect(301, `/${orgSlug}/ele/${page}`);
    });
  }
