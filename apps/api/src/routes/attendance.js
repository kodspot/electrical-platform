...
async function attendanceRoutes(fastify, opts) {
  ...
  fastify.get('/attendance/report/csv', {
    preHandler: [requireRole('ADMIN')]
  }, async (request, reply) => {
    const orgId = request.user.orgId;
    const { from, to, workerId, locationId, shift } = request.query;
    ...
    if (!from || !to || !isValidDateStr(from) || !isValidDateStr(to)) {
      return reply.code(400).send({ error: 'Valid from and to dates required (YYYY-MM-DD)' });
    }
    ...
    // Filter by location through duty rosters (same logic as PDF)
    if (locationId && !workerId) {
      ...
      const rosters = await prisma.dutyRoster.findMany({
        where: rosterWhere,
        select: { workers: { select: { workerId: true } } }
      });
      const rosteredIds = new Set();
      for (const r of rosters) for (const w of r.workers) rosteredIds.add(w.workerId);
      if (rosteredIds.size > 0) {
        where.workerId = { in: [...rosteredIds] };
      } else {
        // No workers in this location for the range — return empty CSV
        const csv = 'Date,Worker,Employee ID,Shift,Status,Marked By,Note';
        return reply
    ...
    const header = 'Date,Worker,Employee ID,Shift,Status,Marked By,Note';
    const rows = records.map(r => [
      r.date.toISOString().split('T')[0],
      esc(r.worker?.name),
      esc(r.worker?.employeeId),
      r.shift,
      r.status,
      esc(r.markedBy?.name),
      esc(r.note)
