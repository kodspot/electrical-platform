...
async function workerRoutes(fastify, opts) {
  ...
  fastify.get('/workers', {
    ...
    const sortMap = {
      ...
      'dept': { department: 'asc' },
      'inspections': { electricalInspections: { _count: 'desc' } }
    };
  ...
  // Worker inspection stats by period
  fastify.get('/workers/:id/stats', {
    ...
    const [daily, weekly, monthly, yearly, total, flagged, lastRecord, shiftRecords, locationRecords] = await Promise.all([
  ...
  fastify.delete('/workers/:id/permanent', {
    ...
    // Disconnect from inspection records, then delete
    await prisma.worker.update({
      where: { id },
      data: { electricalInspections: { set: [] } }
    });
  ...
  // ── Set/Reset Worker PIN (admin only) ──
  fastify.patch('/workers/:id/pin', {
    preHandler: [requireRole('ADMIN')]
  }, async (request, reply) => {
    const schema = z.object({
    ...
    const pinHash = await bcrypt.hash(data.pin, BCRYPT_COST);
    await prisma.worker.update({
      where: { id: worker.id },
      data: { pinHash, tokenInvalidBefore: new Date() }
    });
    ...
    await prisma.auditLog.create({
      data: {
        ...
        entityType: 'Worker',
        entityId: worker.id,
        newValue: { workerName: worker.name }
      }
    });
