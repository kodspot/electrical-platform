...
async function supervisorRoutes(fastify, opts) {
  ...
  fastify.get('/supervisors', async (request) => {
    ...
    const sortMap = {
      'name': { name: 'asc' },
      'name-desc': { name: 'desc' },
      'records': { electricalInspections: { _count: 'desc' } },
    ...
    const select = {
      ...
      _count: { select: { electricalInspections: true, ticketsAssigned: true } }
    };
  ...
  fastify.get('/supervisors/:id/stats', async (request, reply) => {
    ...
    const [daily, weekly, monthly, yearly, total, flagged, lastRecord, shiftRecords, locationRecords, ticketsAssigned, ticketsResolved, ticketsOpen] = await Promise.all([
      prisma.electricalInspection.count({ where: { ...base, inspectedAt: { gte: today } } }),
      prisma.electricalInspection.count({ where: { ...base, inspectedAt: { gte: weekAgo } } }),
      prisma.electricalInspection.count({ where: { ...base, inspectedAt: { gte: monthAgo } } }),
      prisma.electricalInspection.count({ where: { ...base, inspectedAt: { gte: yearAgo } } }),
      prisma.electricalInspection.count({ where: base }),
      prisma.electricalInspection.count({ where: { ...base, status: 'FLAGGED' } }),
      prisma.electricalInspection.findFirst({ where: base, orderBy: { inspectedAt: 'desc' }, select: { inspectedAt: true } }),
      prisma.electricalInspection.groupBy({ by: ['shift'], where: base, _count: true }),
      prisma.electricalInspection.groupBy({ by: ['locationId'], where: base, _count: true, orderBy: { _count: { locationId: 'desc' } }, take: 5 }),
    ...
    return {
      ...
      flagged,
      lastActivity: lastRecord ? lastRecord.inspectedAt : null,
      shifts,
      topLocations,
  ...
  // Permanent delete supervisor (must be deactivated first, no inspection records)
  fastify.delete('/supervisors/:id/permanent', async (request, reply) => {
    ...
    const recordCount = await prisma.electricalInspection.count({ where: { supervisorId: id } });
    if (recordCount > 0) {
      return reply.code(409).send({
        error: 'Cannot permanently delete supervisor with ' + recordCount + ' inspection records. They will remain archived.',
        electricalInspections: recordCount
      });
    }
