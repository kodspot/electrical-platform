...
async function locationRoutes(fastify, opts) {
  ...
  fastify.get('/locations', {
    ...
    const locations = await prisma.location.findMany({
      ...
      include: {
        ...
        _count: { select: { electricalInspections: true, tickets: true } }
      }
    });
  ...
  fastify.delete('/locations/:id', {
    ...
    // Check dependencies
    const [childCount, inspectionCount, ticketCount] = await Promise.all([
      prisma.location.count({ where: { parentId: id } }),
      prisma.electricalInspection.count({ where: { locationId: id } }),
      prisma.ticket.count({ where: { locationId: id } })
    ]);

    if ((childCount > 0 || inspectionCount > 0 || ticketCount > 0) && force !== 'true') {
      return reply.code(409).send({
        error: 'Location has dependencies',
        dependencies: { children: childCount, electricalInspections: inspectionCount, tickets: ticketCount }
      });
    }
